    ...
    { 
        ...
        */
        private static void ImportLinkData(this UrdfLink urdfLink, Link link, Joint joint)
        {
            ...
            {
                // Bug fix - no rigid body defined in URDF
                urdfLink.gameObject.AddComponent<Rigidbody>();

                Debug.LogWarning("No rigidbody associated with link \"" 
                    + urdfLink.gameObject.name + "\" so one has been created.. please check the result is valid.\n"
                    + "In future this should be done by assigning it to \"" + link.name + "\" in the URDF file in order to create a Rigidbody Component.\n", urdfLink.gameObject);
            }
